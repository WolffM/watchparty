...
# Collect candidate source files: mkv and raw mp4 (exclude already transcoded *.wp.mp4 explicitly)
$targets = Get-ChildItem -Path $mediaRoot -Recurse -File -Include *.mkv,*.mp4 -ErrorAction SilentlyContinue |
  Where-Object { $name = $_.Name.ToLower(); -not $name.EndsWith('.wp.mp4') }
...
foreach ($f in $targets) {
  ...
  if ($f.Name.ToLower().EndsWith('.mkv')) {
    $mp4 = ($f.FullName -replace '\.mkv$','') + '.wp.mp4'
  } elseif ($f.Name.ToLower().EndsWith('.mp4')) {
    # Raw mp4 (not already suffixed) -> add suffix once
    $mp4 = ($f.FullName -replace '\.mp4$','') + '.wp.mp4'
  } else { continue }
