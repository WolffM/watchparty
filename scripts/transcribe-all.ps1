...
$defaultInputRoot  = Join-Path $mediaRoot 'anime'
...
if ($Source) {
  ...
  if ($it.PSIsContainer) {
    $targets = Get-ChildItem -Path $it.FullName -Recurse -File -Include *.mkv,*.mp4 | Where-Object { $_.Name -notmatch '\\.wp\\.mp4$' }
  } else {
    $targets = @($it)
  }
  if ($Single -and $targets.Count -gt 1) { $targets = @($targets[0]) }
} else {
  if (-not (Test-Path $defaultInputRoot)) { try { New-Item -ItemType Directory -Path $defaultInputRoot | Out-Null } catch {} }
  $targets = Get-ChildItem -Path $defaultInputRoot -Recurse -File -Include *.mkv,*.mp4 | Where-Object { $_.Name -notmatch '\\.wp\\.mp4$' }
}
...
function Invoke-ExtractSubs {
  ...
  # Count duplicates per language for slugging
  $langCounts = @{}
  for($i=0;$i -lt $streams.Count;$i++){
    $lang=$null; try { $lang=$streams[$i].tags.language } catch {}; if(-not $lang){ $lang='und' } else { $lang=$lang.ToLower() }
  ...
  $core = if($slugExtra){ "$BaseName.$lang.$slugExtra" } else { "$BaseName.$lang" }
    $ext = $SubtitleFormat
    $out = Join-Path $OutputRoot ("$core.$ext")
...
foreach($f in $targets){
  ...
  $videoOut = Join-Path $OutputRoot ($baseName + '.wp.mp4')
  Write-Info "\n==> $($f.Name)"
  if(-not $SkipVideo){
  ...
  if(-not $SkipSubs){ Invoke-ExtractSubs -Src $extractSrc -BaseName $baseName }
}
